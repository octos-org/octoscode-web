 ...
 * long turn looks like. POST /release kills the blocking tool so the held turn
 * finishes on command.
 */
...
createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (url.startsWith("/status")) {
      // `turn/start` returning is NOT the tool running: the agent loop calls
      // the model afterwards. A releaser that does not wait for this counter
      // kills nothing and leaves the turn held forever.
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ holdsIssued }));
      return;
    }
    if (url.startsWith("/release")) {
      let killed = 0;
      try {
        execSync(`pkill -f ${JSON.stringify(SLEEP)}`);
        killed = 1;
      } catch {
        /* nothing was holding */
      }
      console.log(`${ts()} [stub] release: killed=${killed}`);
      res.writeHead(200, { "content-type": "application/json" });
