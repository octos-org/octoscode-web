 ...
 * tab's previous Session, and null means the operator has to press Connect.
 * The entry uses it to decide whether to load the product shell before the
 * first paint; the shell uses the
 * same answer to decide what to send, so the two can never disagree.
 */
export function autoStartKind(input: AutoStartInput): "restore" | null {
  if (input.pairingLink) return null;
  if (!input.restoreConnection) return null;
